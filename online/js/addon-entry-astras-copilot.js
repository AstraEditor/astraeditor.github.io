(window["webpackJsonpGUI"] = window["webpackJsonpGUI"] || []).push([["addon-entry-astras-copilot"],{

/***/ "./src/addons/addons/astras-copilot/_runtime_entry.js":
/*!************************************************************!*\
  !*** ./src/addons/addons/astras-copilot/_runtime_entry.js ***!
  \************************************************************/
...
/* harmony import */ var _userscript_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./userscript.js */ "./src/addons/addons/astras-copilot/userscript.js");
...
/***/ "./src/addons/addons/astras-copilot/userscript.js":
/*!********************************************************!*\
  !*** ./src/addons/addons/astras-copilot/userscript.js ***!
  \********************************************************/
  ...
  (function () {
    ...
    const ScratchDSL = function () {
      ...
      function getShadowType(blockType, valueName) {
        const key = "".concat(blockType, "__").concat(valueName);
      ...
      function createDefaultShadow(doc, blockType, valueName) {
      ...
      function setValueWithAutoShadow(doc, blockEl, valueName, valueStr) {
        let valueEl = blockEl.querySelector("value[name=\"".concat(valueName, "\"]"));
        if (!valueEl) {
            ...
            case 'VALUE':
              {
                if (parent.type === 'block') {
                  ...
                  let valueEl = parent.element.querySelector("value[name=\"".concat(valueName, "\"]"));
                  if (!valueEl) {
                    valueEl = doc.createElement('value');
                    valueEl.setAttribute('name', valueName);
      ...
      function loadToWorkspace(dsl, workspace) {
        ...
        try {
          ...
          return {
            success: true,
            blockCount: targetWorkspace.getAllBlocks().length
          };
        } catch (e) {
    ...
    async function callOpenAI(messages) {
      ...
      let modelOverride = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      let model = modelOverride || config.model;
      if (!model || model.trim() === '') throw new Error('Model name is empty');
      model = model.trim();
      const url = replacePlaceholders(config.apiUrl, {
      ...
      if (!response.ok) {
        ...
        throw new Error("HTTP ".concat(response.status, ": ").concat(text.slice(0, 200)));
      }
      const data = await response.json();
      return ((_data$choices = data.choices) === null || _data$choices === void 0 ? void 0 : (_data$choices$ = _data$choices[0]) === null || _data$choices$ === void 0 ? void 0 : (_data$choices$$messag = _data$choices$.message) === null || _data$choices$$messag === void 0 ? void 0 : _data$choices$$messag.content) || 'No response';
    }
    ...
    function injectAstrasCopilotButton() {
      ...
      chatButton.style.cssText = "\n            width: ".concat(getComputedStyle(feedbackButton).width, ";\n            height: ").concat(getComputedStyle(feedbackButton).height, ";\n            background-color: var(--looks-secondary);\n            color: white;\n            border: none;\n            margin-left: 10px;\n            border-radius: 5px;\n            cursor: pointer;\n            font-weight: bold;\n            transition: transform 0.2s, box-shadow 0.2s;\n            ");
      ...
      function createImportButton(dslCode) {
        ...
        btn.onclick = () => {
          ...
          const result = ScratchDSL.loadToWorkspace(dslCode, null, {
            clear: false,
            center: true
          });
          if (result.success) {
            btn.innerText = "\u2705 Imported ".concat(result.blockCount, " blocks!");
      ...
      function openPopup() {
        ...
        popupContainer.style.cssText = "\n                position: fixed;\n                top: ".concat(localStorage.getItem('astras_popupTop') || '50px', ";\n                left: ").concat(localStorage.getItem('astras_popupLeft') || '50px', ";\n                background-color: ").concat(getCurrentTheme() === 'dark' ? '#2c2c2c' : 'white', ";\n                padding: 0;\n                border-radius: 15px;\n                box-shadow: 0 10px 40px rgba(0,0,0,0.3);\n                z-index: 9999;\n                width: 500px;\n                height: 600px;\n                overflow: hidden;\n                display: flex;\n                flex-direction: column;\n                border: 2px solid var(--looks-secondary);\n                ");
        ...
        chatWindow.style.cssText = "\n                flex: 1;\n                width: 100%;\n                overflow-y: auto;\n                padding: 15px;\n                box-sizing: border-box;\n                background-color: ".concat(getCurrentTheme() === 'dark' ? '#1a1a1a' : '#f8f9fa', ";\n                color: ").concat(getCurrentTheme() === 'dark' ? '#fff' : '#000', ";\n                ");
        ...
        inputArea.style.cssText = "\n                padding: 15px;\n                background-color: ".concat(getCurrentTheme() === 'dark' ? '#2c2c2c' : '#fff', ";\n                border-top: 1px solid ").concat(getCurrentTheme() === 'dark' ? '#444' : '#e0e0e0', ";\n                display: flex;\n                flex-direction: column;\n                gap: 10px;\n                ");
        ...
        modelInput.style.cssText = "\n                flex: 1;\n                padding: 6px 10px;\n                border: 1px solid ".concat(getCurrentTheme() === 'dark' ? '#555' : '#ddd', ";\n                border-radius: 5px;\n                background: ").concat(getCurrentTheme() === 'dark' ? '#3a3a3a' : '#fff', ";\n                color: inherit;\n                font-size: 12px;\n                ");
        modelInput.onchange = () => {
        ...
        clearBtn.style.cssText = "\n                flex: 1;\n                padding: 10px;\n                background: ".concat(getCurrentTheme() === 'dark' ? '#444' : '#e0e0e0', ";\n                color: inherit;\n                border: none;\n                border-radius: 8px;\n                cursor: pointer;\n                ");
        clearBtn.onclick = () => {
          chatMessages = [];
        ...
        genBtn.onclick = () => {
          const ideas = ["Make a sprite bounce around the screen", "Create a simple calculator with variables", "Make a sprite follow the mouse", "Create a countdown timer", "Make a sprite draw a spiral"];
        ...
        async function sendMessage() {
          ...
          userDiv.innerHTML = "<div style=\"color:var(--looks-secondary); font-weight:bold; margin-bottom:5px;\">You:</div><div style=\"background:".concat(getCurrentTheme() === 'dark' ? '#333' : '#e3f2fd', "; padding:10px; border-radius:10px; border-bottom-left-radius:2px;\">").concat(formatText(text), "</div>");
          ...
          typingDiv.innerHTML = "<div style=\"color:#f5576c; font-weight:bold; margin-bottom:5px;\">AstrasCopilot:</div><div style=\"background:".concat(getCurrentTheme() === 'dark' ? '#333' : '#fff3e0', "; padding:10px; border-radius:10px; border-bottom-right-radius:2px;\">Thinking...</div>");
          chatWindow.appendChild(typingDiv);
          chatWindow.scrollTop = chatWindow.scrollHeight;
          try {
            const messages = [{
              role: 'system',
              content: SCRATCHDSL_SYSTEM_PROMPT
            ...
            if (dslCode) {
              contentHtml += "<div style=\"margin-top:10px; padding:10px; background:".concat(getCurrentTheme() === 'dark' ? '#1e3a5f' : '#e8f4f8', "; border-radius:8px; border-left:4px solid var(--looks-secondary);\"><div style=\"font-size:11px; color:var(--looks-secondary); margin-bottom:5px; font-weight:bold;\">\uD83E\uDDE9 ScratchDSL Code Detected</div></div>");
            }
            aiDiv.innerHTML = "<div style=\"color:#f5576c; font-weight:bold; margin-bottom:5px;\">AstrasCopilot:</div><div style=\"background:".concat(getCurrentTheme() === 'dark' ? '#333' : '#fff3e0', "; padding:10px; border-radius:10px; border-bottom-right-radius:2px;\">").concat(contentHtml, "</div>");
            if (dslCode) {
            ...
            errorDiv.innerHTML = "<strong>Error:</strong> ".concat(error.message);
      ...
      function formatText(text) {
        ...
        // Format markdown
        text = text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\*(.*?)\*/g, '<em>$1</em>').replace(/`([^`]+)`/g, '<code style="background:rgba(0,0,0,0.1); padding:2px 4px; border-radius:3px; font-family:monospace;">$1</code>').replace(/```[\s\S]*?```/g, match => "<pre style=\"background:rgba(0,0,0,0.05); padding:10px; border-radius:5px; overflow-x:auto; font-size:12px;\">".concat(match.replace(/```/g, ''), "</pre>"));
    ...
    function openSettingsPopup() {
      ...
      document.getElementById('saveSettings').onclick = () => {
        ...
        config.headers = headersArray.length ? headersArray : [{
          key: 'Authorization',
          value: 'Bearer {{apiKey}}'
        }];
...
//# sourceMappingURL=addon-entry-astras-copilot.js.map