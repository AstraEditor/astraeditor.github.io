...
/* harmony default export */ __webpack_exports__["default"] = (async function () {
  ...
  (function () {
    ...
    function migrateOldStorage() {
      ...
      let migrated = false;
      Object.entries(oldKeys).forEach(_ref => {
        let [prop, oldKey] = _ref;
    ...
    async function callOpenAI(messages) {
      var _data$choices, _data$choices$, _data$choices$$messag, _data$choices2, _data$choices2$;
      ...
      config.headers.forEach(h => {
        let value = replacePlaceholders(h.value, {
          apiKey: config.apiKey,
          model
        });
        headers[h.key] = value;
      ...
      try {
        ...
        throw new Error("\u274C Invalid body template JSON: ".concat(e.message));
      }
      ...
      if (!contentType.includes('application/json')) {
        const text = await response.text();
        throw new Error("Response is not JSON (".concat(contentType, "): ").concat(text.slice(0, 200)));
      }
      const data = await response.json();
      return ((_data$choices = data.choices) === null || _data$choices === void 0 ? void 0 : (_data$choices$ = _data$choices[0]) === null || _data$choices$ === void 0 ? void 0 : (_data$choices$$messag = _data$choices$.message) === null || _data$choices$$messag === void 0 ? void 0 : _data$choices$$messag.content) || ((_data$choices2 = data.choices) === null || _data$choices2 === void 0 ? void 0 : (_data$choices2$ = _data$choices2[0]) === null || _data$choices2$ === void 0 ? void 0 : _data$choices2$.text) || 'No response content.';
    }
    ...
    function injectAstrasCopilotButton() {
          ...
          popup.querySelector('#analyzeConfirm').addEventListener('click', async function () {
            ...
            try {
              ...
              reader.onload = async function (e) {
                ...
                const model = ((_document$getElementB = document.getElementById('modelInput')) === null || _document$getElementB === void 0 ? void 0 : _document$getElementB.value.trim()) || config.model;
                if (!model) {
      ...
      function formatText(text) {
        text = text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\*(.*?)\*/g, '<em>$1</em>').replace(/`([^`]+)`/g, '<code>$1</code>').replace(/"(.*?)"/g, '<span style="color: #639cff; font-style: italic;">"$1"</span>');
        let blocks = text.split(/\n\s*\n/);
        blocks = blocks.map(block => {
          let lines = block.split('\n').map(l => l.trim()).filter(l => l);
          if (lines.every(l => /^\d+\./.test(l))) {
            let items = lines.map(l => "<li>".concat(l.replace(/^\d+\.\s*/, ''), "</li>"));
            return "<ol>".concat(items.join(''), "</ol>");
          } else if (lines.every(l => /^[-*]\s/.test(l))) {
            let items = lines.map(l => "<li>".concat(l.replace(/^[-*]\s*/, ''), "</li>"));
            return "<ul>".concat(items.join(''), "</ul>");
          } else {
            return "<p>".concat(block.replace(/\n/g, '<br>'), "</p>");
          }
        });
        return blocks.join('');
      }
